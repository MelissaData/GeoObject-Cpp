# Name:    MelissaGeoCoderObjectWindowsCpp
# Purpose: Use the MelissaUpdater to make the MelissaGeoCoderObjectWindowsCpp code usable
...
$ProjectPath = "$CurrentPath\MelissaGeoCoderObjectWindowsCpp"
...
function CheckDLLs() {
  ...
  if (!(Test-Path ("$ProjectPath\mdGeo.h"))) {
    Write-Host "mdGeo.h not found." 
    $FileMissing = $true
  }
  if (!(Test-Path ("$ProjectPath\mdGeo.lib"))) {
...
Write-Host "`n===================== Melissa GeoCoder Object =====================`n                     [ C++ | Windows | 64BIT ]`n"
...
if ([string]::IsNullOrEmpty($zip)) {
  & $BuildPath\MelissaGeoCoderObjectWindowsCpp.exe --license $License --dataPath $DataPath
}
else {
  & $BuildPath\MelissaGeoCoderObjectWindowsCpp.exe --license $License --dataPath $DataPath --zip $zip
}
